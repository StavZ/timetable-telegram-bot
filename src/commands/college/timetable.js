...
export default class TimetableC extends Command {
  ...
  async exec(ctx) {
    ...
    this.client.action('show-remote-works', async (ctxu) => {
      ...
      const rw = await this.client.rw.getByDate(user.group, this.client.moment().format('DD-MM-YY'));
  ...
  async showUserSchedule(ctx, key, editId) {
    ...
    if (userTimetable.lessons.filter((l) => l.isRemote).length) {
      keyboard.push([{ text: 'Показать дистанционные задания', callback_data: 'show-remote-works' }]);
    }
    keyboard.push([{ text: 'Убрать кнопки', callback_data: 'cancel-timetable' }]);

    if (editId) {
      this.client.telegram
        .editMessageText(ctx.chat.id, editId, editId, msg, { reply_markup: { inline_keyboard: keyboard, one_time_keyboard: true }, disable_web_page_preview: true, parse_mode: 'Markdown' })
  ...
  async selectGroup(ctx) {
    ...
    return ctx.editMessageText('Выберите группу из списка ниже:', { reply_markup: { inline_keyboard: keyboard, one_time_keyboard: true } });
  }
